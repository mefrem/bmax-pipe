...
export async function orchestrateRepository({
  ...
  try {
    ...
    const githubOrg = env.GITHUB_ORG && env.GITHUB_ORG.trim() !== "" ? env.GITHUB_ORG : undefined;

    console.log("[Orchestrator] Fetching authenticated user...");
    const owner = githubOrg ?? (await octokit.users.getAuthenticated()).data.login;
    console.log("[Orchestrator] Owner:", owner, githubOrg ? "(org)" : "(personal)");
    ...
    if (githubOrg) {
      await octokit.repos.createInOrg({
        ...
        private: true,
        auto_init: true,
        description: `Generated by bmax-pipe for ${userEmail}`,
      });
    } else {
